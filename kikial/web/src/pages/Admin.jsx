...
export default function AdminPage({ user, onBack, onLogout }) {
  ...
  async function removeKnowledge(id) {
    try {
      ...
      setItems((current) => current.filter((item) => item.learnedAt !== id));
    } catch (err) {
        ...
        {loading ? (
          ...
          <div style={{ display: "grid", gap: 12 }}>
            {items.map((item, index) => (
              <article key={item.learnedAt || `${item.question}-${index}`} style={{ background: "#111827", border: "1px solid #374151", borderRadius: 16, padding: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "flex-start" }}>
                  <div style={{ flex: 1 }}>
                    <p style={{ margin: "0 0 8px", color: "#a78bfa", fontWeight: 700 }}>Câu hỏi #{index + 1}</p>
                  ...
                  <button
                    type="button"
                    onClick={() => removeKnowledge(item.learnedAt)}
